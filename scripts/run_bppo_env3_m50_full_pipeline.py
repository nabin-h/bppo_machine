...
def ensure_existing_manifest(dataset_root: Path, num_machines: int, thresholds, random_fraction: float):
    tag = mix_tag(thresholds, random_fraction)
    manifest_path = dataset_root / f"dataset_manifest_MM{num_machines}_{tag}.json"
    if not manifest_path.exists():
        raise FileNotFoundError(
            f"Expected dataset manifest at {manifest_path}. Copy the DT-created M{num_machines} dataset into this repo first."
        )
...
def build_config_payload(env_name: str, manifest_path: Path, dataset_root_rel: str, policy_paths, device: str):
    ...
    return {
        ...
        "repo_root": ".",
        "output_root": "Seed runs/M50 runs/runs_mm50_env3_567_r20_bppo_seedruns",
...
def main():
    parser = argparse.ArgumentParser(description="Full BPPO M50 env3 pipeline using an existing copied dataset.")
    ...
    parser.add_argument("--dataset_root", type=str, default="datasets_mm50_env3_mix567_r20")
    ...
    manifest_path, tag = ensure_existing_manifest(dataset_root, args.num_machines, thresholds, args.random_fraction)
    manifest_path = ensure_compat_manifest(manifest_path, args.num_machines, tag)

    config_path = generated_config_dir / "mm50_env3_567_r20_seedruns.json"
    ...
    output_root = repo_root / args.base_output_dir / "M50 runs" / "runs_mm50_env3_567_r20_bppo_seedruns"
    cmd = [
        sys.executable,
